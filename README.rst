...
at the computer and take my wife out on a date night, then consider sending
...
It is highly recommend to store the ``AuthorizeNet_AIM_9`` type in a location
on your web server that cannot be served, preferably outside of your web root.
...
#. payment gateway (an Authorize.Net account, for example). I am an
   ...
   of the ``AuthorizeNet_AIM_9`` method by following my `reseller link
